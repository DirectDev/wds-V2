...
class AdminFacebookServices {
    ...
    public function __construct(EntityManager $em, $securityContext, $facebook_app_id, $facebook_app_secret, $facebook_user_id, $locales) {
        $this->em = $em;
        $this->user  = $this->em->getRepository('UserUserBundle:User')->findOneBy(array('facebook_id' => $facebook_user_id));
        if ($this->user)
    ...
    private function allowImportEvent() {
        if ($this->eventNode->getField('type') != 'public') return false;

        if ($this->eventNode->getField('end_time')) {
            $now = new \DateTime();
            $end = $this->eventNode->getField('end_time');
            if ($end->format('U') < $now->format('U')) return false;
        }

        if ($this->eventNode->getField('start_time')) {
            $now   = new \DateTime();
            $start = $this->eventNode->getField('start_time');
            if ($start->format('U') < $now->format('U')) return false;
        }

        foreach ($this->music_array as $music_title) {
            if (stripos($this->eventNode->getField('name'),
                    $music_title) !== false) return true;
            if (stripos($this->eventNode->getField('description'),
                    $music_title) !== false) return true;
        }
    ...
    private function setEventAddress() {
        ...
        if (!$address) $address = $this->createAddress();
        if ($address) {
            foreach ($this->event->getAddresses() as $event_address) {
                if ($event_address == $address) return;
            }
    ...
    private function setEventPictureUrl() {
        if (!$this->facebook or ! $this->facebook_event_id) return;
    ...
    private function createEventFile() {
        if (!$this->event->getFacebookPictureUrl()) return;
        ...
        try {
            ...
            if (!$eventFile) $eventFile = new EventFile();
            ...
            if (stripos($url,
                    '.png') !== false) $name = 'img_facebook.png';
            if (stripos($url,
                    '.gif') !== false) $name = 'img_facebook.gif';
    ...
    private function findUserByFacebookOwnerOrFacebookAdmins() {
        ...
        if ($owner) return $this->findUserByFacebookId($owner->getField('id'));
    ...
    private function isInFacebookOwnerOrFacebookAdmins($facebook_id = null) {
        ...
        if ($owner) {
            ...
            if ($facebook_id == $owner_id) return true;
        }
        $admins = $this->getNodeData('admins');
        if ($admins)
                foreach ($admins->getIterator() as $item) {
                $admin_id = $item->getField('id');
                if ($facebook_id == $admin_id) return true;
            }
        return false;
    }
    ...
    public function searchEventsByCity($city_name = null) {
        ...
        $this->setMusicArray();
        $q = '&q='.$city_name.',{'.implode('|',
                $this->music_array).'}';

        $fields         = 'id,description,name,type,place,cover,start_time,end_time,admins,attending';
        $since          = 'since='.date('U');
        $until          = 'until='.(date('U') + 604800 * 2);
        $fb_request_url = '/search?type=event&'.$since.'&'.$until.'&fields='.$fields.$q.'&limit='.$this->limit_add;
        ...
        foreach ($this->graphEdge->getIterator() as $eventNode) {
            ...
            if (!$this->allowImportEvent()) continue;
            $add++;
            if ($add > $this->limit_add) continue;
